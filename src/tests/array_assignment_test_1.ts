...
export class ArrayAssignments {
    ...
    @test "should automatically promote real arrays to complex ones"() {
        ...
        Set(a, [mks(1)], mkc(1, 1));
        assert.deepEqual(Get(a, [1]), mkc(1, 1));
        for (let i = 2; i <= 10; i++) {
            assert.deepEqual(Get(a, [i]), mkc(i, 0));
        }
    }
        ...
        for (let i = 1; i <= 5; i++) {
            for (let j = 1; j <= 4; j++) {
                for (let k = 1; k <= 3; k++) {
                    ...
                    a = Set(a, [mks(k), mks(j), mks(i)], mks(p));
                }
            }
        }
        for (let i = 1; i <= 5; i++) {
            for (let j = 1; j <= 4; j++) {
                for (let k = 1; k <= 3; k++) {
                    let p = k + (j - 1) * 3 + (i - 1) * 4 * 3;
                    assert.deepEqual(Get(a, [k,j,i]), mks(p));
                }
            }
        }
    }
    @test "should allow for multidimensional gets/sets in a multidim array (complex)"() {
        ...
        for (let i = 1; i <= 5; i++) {
            for (let j = 1; j <= 4; j++) {
                for (let k = 1; k <= 3; k++) {
                    let p = k + (j - 1) * 3 + (i - 1) * 4 * 3;
                    a = Set(a, [mks(k), mks(j), mks(i)], mkc(p, -p));
                }
            }
        }
        for (let i = 1; i <= 5; i++) {
            for (let j = 1; j <= 4; j++) {
                for (let k = 1; k <= 3; k++) {
                    let p = k + (j - 1) * 3 + (i - 1) * 4 * 3;
                    assert.deepEqual(Get(a, [k,j,i]), mkc(p, -p));
                }
            }
        }
    }
    ...
    @test "should allow for column addressing for gets in a multidimensional array"() {
        ...
        for (let i = 1; i <= (3 * 4 * 5); i++) {
            assert.deepEqual(Get(a, [i]), mks(i));
        }
    }
