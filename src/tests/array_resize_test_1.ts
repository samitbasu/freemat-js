...
import { Resize, realScalar, Get, FMArray, Set } from "../arrays";

import { rand_array, test_mat, test_mat_complex, mks } from "./test_utils";
...
export class ResizeTests {
        ...
        for (let dim of sizes) {
            ...
            for (let row = 1; row <= dim; row++)
                for (let col = 1; col <= dim; col++)
                    assert.deepEqual(Get(P, [row, col]), Get(Q,[row, col]));
            ...
            for (let slice = 1; slice <= 3 * dim; slice += 8)
                for (let col = 1; col <= 2 * dim; col += 3)
                    for (let row = 1; row <= dim; row += 4)
                        assert.deepEqual(Get(P, [row, col, slice]), Get(Q, [row, col, slice]));
    ...
    @test "should correctly vector-resize a matrix"() {
        ...
        for (let ndx = 1; ndx <= 12; ndx++) {
            assert.equal(realScalar(Get(P, [mks(ndx)])), ndx);
        }
    }
