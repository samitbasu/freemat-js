...
describe('mldivide tests', function() {
    let sizes = [2, 4, 8, 32, 100, 200];
    for (let dim of sizes) {
        it(`should correctly solve A\\b for real square matrices of size ${dim} x ${dim}`, () => {
    ...
    for (let dim of [2,4,8,32,64,128]) {
        it(`should correctly solve A\\b for complex matrices of size ${dim} x ${dim}`, () => {
            let C = dbl.make_array([dim,dim]);
            for (let i=1;i<=dim;i++) {
    ...
    for (let dim of sizes) {
        it(`should correctly solve A\\b for real rectangular matrices of size ${2*dim} x ${dim}`, () => {
    ...
    for (let dim of sizes) {
        it(`should correctly solve A\\B for real rectangular matrices of size ${2*dim} x ${dim}, and right hand sides of size ${dim} x 4`, () => {
            let C = dbl.make_array([2*dim,dim]);
            for (let i=1;i<=dim;i++) {
                C = C.set([i,i],1);
                C = C.set([i+dim,i],1);
    ...
    for (let dim of sizes) {
        it(`should correctly solve A\\b for real rectangular matrices of size ${dim} x ${2*dim}, and right hand vectors`, () => {
    ...
    it('should refuse to compute A\\b if A and b do not have the same number of rows', () => {
	let C = dbl.make_array([7,9]);
	let B = dbl.make_array([8,3]);
	assert.throws(()=>{D=C.mldivide(B);},TypeError,/conformant/);
    });
});
